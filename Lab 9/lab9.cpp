...
inline void send_work(vector <int> &a, vector <int> &b, int nrProcs) {
  ...
  for(int i = 1; i < nrProcs; ++ i) {
    ...
    MPI_Bsend(&n,  1, MPI_INT, i, 0, MPI_COMM_WORLD);   //Basic send with user-provided buffering
...
int main(int argc, char** argv) {
  ...
  int me;
  int nrProcs;
  MPI_Comm_size(MPI_COMM_WORLD, &nrProcs);    //Determines the size of the group associated with a communicator
  MPI_Comm_rank(MPI_COMM_WORLD, &me);       //Determines the rank of the calling process in the communicator
